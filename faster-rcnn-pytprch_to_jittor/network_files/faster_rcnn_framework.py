...
from MutilscaleRoIAlign import MultiScaleRoIAlign
...
class FasterRCNNBase(nn.Module):
    ...
    def eager_outputs(self, losses, detections):
        # type: (Dict[str, Var], List[Dict[str, Var]]) -> Union[Dict[str, Var], List[Dict[str, Var]]]
        if self.training:
            return losses
    ...
    def execute(self, images, targets=None):
        # type: (List[Var], Optional[List[Dict[str, Var]]]) -> Tuple[Dict[str, Var], List[Dict[str, Var]]]
        """
        Arguments:
        ...
        original_image_sizes: List[Tuple[int, int]] = [] # 图像的原始的尺寸
        for img in images:
            val = img.shape[-2:]
        ...
        if isinstance(features, jt.Var):  # 若只在一层特征层上预测，将feature放入有序字典中，并编号为‘0’
