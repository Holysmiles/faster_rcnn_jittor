...
def _resize_image(image, self_min_size, self_max_size):
    # type: (Var, float, float) -> Var
    im_shape = jt.array(image.shape[-2:])
    min_size = float(im_shape.min())    # 获取高宽中的最小值
    max_size = float(im_shape.max())    # 获取高宽中的最大值
    ...
    #     image.unsqueeze(0),  # 替代image[None]，增加batch维度
    #     scale_factor=scale_factor,
    #     mode="bilinear",
    #     recompute_scale_factor=True,
    #     align_corners=False
    # ).squeeze(0)  # 替代[0]，移除batch维度
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def normalize(self, image):
        ...
        mean = jt.array(self.image_mean, dtype=dtype)
        std = jt.array(self.image_std, dtype=dtype)
    ...
    def torch_choice(self, k):
        ...
        is fixed.
        """
        index = int(jt.empty(1).uniform_(0., float(len(k))).item())
        return k[index]
    ...
    def execute(self,
                images,       # type: List[Var]
                targets=None  # type: Optional[List[Dict[str, Var]]]
                ):
        # type: (...) -> Tuple[ImageList, Optional[List[Dict[str, Var]]]]
...
def resize_boxes(boxes, original_size, new_size):
    # type: (Var, List[int], List[int]) -> Var
    """
    ...
    """
    ratios = [
        jt.array(s, dtype='float32') /
        jt.array(s_orig, dtype='float32')
