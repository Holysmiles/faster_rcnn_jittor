...
from network_files import MutilscaleRoIAlign
...
def create_model(num_classes):
    ...
    roi_pooler = MutilscaleRoIAlign(featmap_names=['0'],  # 在哪些特征层上进行roi pooling
...
def main():
    ...
    if aspect_ratio_group_factor >= 0:
        train_sampler = jt.dataset.RandomSampler(train_dataset)
        # train_sampler = jt.RandomSampler(train_dataset)
    ...
    if train_sampler:
        ...
        train_data_loader = jt.dataset.DataLoader(train_dataset,
    ...
    optimizer = jt.optim.SGD(params, lr=0.005,
    ...
    jt.save(model.state_dict(), "./save_weights/pretrain.pth")
    ...
    lr_scheduler = jt.lr_scheduler.StepLR(optimizer,
                                                   step_size=3,
                                                   gamma=0.33)
    num_epochs = 20
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            jt.save(save_files, "./save_weights/mobile-model-{}.pth".format(epoch))
