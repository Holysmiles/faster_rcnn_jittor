...
# from jittor import functional as F
...
class AnchorsGenerator(nn.Module):
    __annotations__ = {
        "cell_anchors": Optional[List[Var]],
        "_cache": Dict[str, List[Var]]
    }
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=jt.float32, device="gpu"):
        # type: (List[int], List[float], jt.float32, 'gpu') -> Var
        """
    ...
    def grid_anchors(self, grid_sizes, strides):
        # type: (List[List[int]], List[List[Var]]) -> List[Var]
        """
    ...
    def execute(self, image_list, feature_maps):
        # type: (ImageList, List[Var]) -> List[Var]
        ...
        # 获取变量类型和设备类型
        dtype, device = feature_maps[0].dtype, 'gpu'

        # one step in feature map equate n pixel stride in origin image
        # 计算特征层上的一步等于原始图像上的步长
        strides = [[jt.array(image_size[0] // g[0], dtype=jt.int64),
                    jt.array(image_size[1] // g[1], dtype=jt.int64)] for g in grid_sizes]
        ...
        anchors = [jt.concat(anchors_per_image) for anchors_per_image in anchors]
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        for layer in self.children():
            if isinstance(layer, nn.Conv2d):
                # w = jt.empty(layer.weight.shape)
                # jt.normal(size=w, std=0.01,mean=0.0)
                jt.nn.init.gauss_(layer.weight, std=0.01)
                jt.nn.init.constant_(layer.bias, 0)

    def execute(self, x):
        # type: (List[Var]) -> Tuple[List[Var], List[Var]]
        logits = []
        bbox_reg = []
        for i, feature in enumerate(x):
            t = jt.nn.relu(self.conv(feature))
...
def permute_and_flatten(layer, N, A, C, H, W):
    # type: (Var, int, int, int, int, int) -> Var
    """
...
def concat_box_prediction_layers(box_cls, box_regression):
    # type: (List[Var], List[Var]) -> Tuple[Var, Var]
    """
    ...
    box_cls = jt.concat(box_cls_flattened, dim=1).flatten(0, -2)  # start_dim, end_dim
...
class RegionProposalNetwork(nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        # type: (List[Var], List[Dict[str, Var]]) -> Tuple[List[Var], List[Var]]
        """
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                matched_gt_boxes_per_image = jt.zeros(anchors_per_image.shape, dtype='float32')
                labels_per_image = jt.zeros((anchors_per_image.shape[0],), dtype='float32')
            else:
                ...
                labels_per_image = labels_per_image.to(dtype=jt.float32)
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        for ob in objectness.split(num_anchors_per_level, 1):
            ...
            _, top_n_idx = jt.topk(ob, k=pre_nms_top_n, dim=1)
            r.append(top_n_idx + offset)
            offset += num_anchors
        return jt.concat(r, dim=1)

    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        # type: (Var, Var, List[Tuple[int, int]], List[int]) -> Tuple[List[Var], List[Var]]
        """
        ...
        levels = [jt.full((n, ), idx, dtype='int64')
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            ...
            keep = jt.where(scores >= self.score_thresh)[0]  # ge: >=
            boxes, scores, lvl = boxes[keep], scores[keep], lvl[keep]
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        # type: (Var, Var, List[Var], List[Var]) -> Tuple[Var, Var]
        """
    ...
    def forward(self,
                ...
                features,      # type: Dict[str, Var]
                targets=None   # type: Optional[List[Dict[str, Var]]]
                ):
        # type: (...) -> Tuple[List[Var], Dict[str, Var]]
        """
        Arguments:
