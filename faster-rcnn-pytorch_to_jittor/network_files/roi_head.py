...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    # type: (jt.Var, jt.Var, List[jt.Var], List[jt.Var]) -> Tuple[jt.Var, jt.Var]
    """
    Computes the loss for Faster R-CNN.

    Arguments:
        class_logits : 预测类别概率信息，shape=[num_anchors, num_classes]
        box_regression : 预测边界框回归信息
        labels : 真实类别信息
        regression_targets : 真实目标边界框信息

    Returns:
        classification_loss (jt.Var)
        box_loss (jt.Var)
    """
    # 合并所有batch中的标签和回归目标
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        # type:(List[jt.Var], List[jt.Var], List[jt.Var]) -> Tuple[List[jt.Var], List[jt.Var]]
        """
           为每个proposal匹配对应的gt_box，并划分到正负样本中
           Args:
               proposals: 提案框列表，每个元素是形状为[N,4]的张量
               gt_boxes: 真实框列表，每个元素是形状为[M,4]的张量
               gt_labels: 真实标签列表，每个元素是形状为[M]的张量
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                ...
                clamped_matched_idxs_in_image = jt.zeros(
                    (proposals_in_image.shape[0],),  dtype='int64'
                )
    ...
    def check_targets(self, targets):
        # type: (Optional[List[Dict[str, Var]]]) -> None
    ...
    def select_training_samples(self,
                                proposals,  # type: List[Var]
                                targets     # type: Optional[List[Dict[str, Var]]]
                                ):
        # type: (...) -> Tuple[List[Var], List[Var], List[Var]]
        """
        ...
        gt_boxes = [t["boxes"].astype(dtype) for t in targets]
    ...
    def postprocess_detections(self,
                               class_logits,    # type: Var
                               box_regression,  # type: Var
                               proposals,       # type: List[Var]
        ...
        # 对预测类别结果进行softmax处理
        pred_scores = nn.softmax(class_logits, -1)
    ...
    def execute(self,
        ...
        # type: (...) -> Tuple[List[Dict[str, Var]], Dict[str, Var]]
        """
        Arguments:
        ...
        if targets is not None:
            for t in targets:
                floating_point_types = (jt.float32, jt.float64, jt.float16)
                assert t["boxes"].dtype in floating_point_types, "target boxes must of float type"
                assert t["labels"].dtype == jt.int64, "target labels must of int64 type"
