...
class BalancedPositiveNegativeSampler(object):
    ...
    def __init__(self, batch_size_per_image, positive_fraction):
        ...
        """
        参数:
            batch_size_per_image (int): 每张图像选择的样本总数
            positive_fraction (float): 批次中正样本的比例
        """
    ...
    def __call__(self, matched_idxs):
        ...
        参数:
            matched_idxs: 包含-1、0或正值的张量列表。
                每个张量对应一张图像。
                -1值被忽略，0被视为负样本，>0被视为正样本。

        返回:
            pos_idx (list[tensor]): 正样本的掩码列表
            neg_idx (list[tensor]): 负样本的掩码列表
        """
        pos_idx = []
        neg_idx = []
        # 遍历每张图像的matched_idxs
        for matched_idxs_per_image in matched_idxs:
            ...
            positive = jt.where( matched_idxs_per_image >= 1)[0]  # 正样本索引
            # = 0的为负样本
            # negative = torch.nonzero(matched_idxs_per_image == 0).squeeze(1)
            negative = jt.where( matched_idxs_per_image == 0)[0]  # 负样本索引
            ...
            pos_idx_per_image_mask = jt.zeros_like(
                matched_idxs_per_image, dtype=jt.uint8
            )
...
def encode_boxes(reference_boxes, proposals, weights):
    # type: (jt.Var, jt.Var, jt.Var) -> jt.Var
    ...
    # 扩展维度（Jittor的unsqueeze与PyTorch用法一致）
...
class BoxCoder:
    ...
    def encode(self, reference_boxes, proposals):
        # type: (List[Var], List[Var]) -> List[Var]
        """
        结合anchors和与之对应的gt计算regression参数
        Args:
            reference_boxes: List[Var] 每个proposal/anchor对应的gt_boxes
            proposals: List[Var] anchors/proposals
        ...
        return jt.split(targets, boxes_per_image, 0)
    ...
    def decode(self, rel_codes, boxes):
        ...
        assert isinstance(rel_codes, jt.Var)
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        Arguments:
            rel_codes (jt.Var): encoded boxes (bbox regression parameters)
            boxes (jt.Var): reference boxes (anchors/proposals)
        """
        ...
        # limit max value, prevent sending too large values into jt.exp()
        ...
        pred_w = jt.exp(dw) * widths[:, None]
        pred_h = jt.exp(dh) * heights[:, None]

        # xmin
        pred_boxes1 = pred_ctr_x - jt.array(0.5, dtype=pred_ctr_x.dtype) * pred_w
        # ymin
        pred_boxes2 = pred_ctr_y - jt.array(0.5, dtype=pred_ctr_y.dtype) * pred_h
        # xmax
        pred_boxes3 = pred_ctr_x + jt.array(0.5, dtype=pred_ctr_x.dtype) * pred_w
        # ymax
        pred_boxes4 = pred_ctr_y + jt.array(0.5, dtype=pred_ctr_y.dtype) * pred_h

        pred_boxes = jt.stack((pred_boxes1, pred_boxes2, pred_boxes3, pred_boxes4), dim=2).flatten(1)
        return pred_boxes
...
class Matcher( object):
    ...
    def set_low_quality_matches_(self, matches, all_matches, match_quality_matrix):
        ...
        gt_pred_pairs_of_highest_quality = jt.where(
            jt.equal(match_quality_matrix, highest_quality_foreach_gt[:, None])
        )
