...
from utils.dataloader import FRCNNDataset, init_dataloader, generate_batch
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = '/root/autodl-tmp/faster-rcnn-pytorch-master/model_data/voc_weights_vgg.pth'
    #------------------------------------------------------#
    ...
    if True:
        ...
        # train_dataset, val_dataset = init_dataloader(train_lines, val_lines)
# =========================================================================================================================================
#         from jittor.dataset import BatchSampler, SequentialSampler
        
#         gen = generate_batch(train_dataset, batch_size=8)
#         gen_val = generate_batch(val_dataset, batch_size=8)
        
#         gen_t = jt.array(list(iter(gen)))
#         # raise ValueError(f"============gen:{gen_t.shape}")

        # DataLoader 创建
        input_shape = (400, 400)  # 根据实际需求设置
        batch_size = 8
    
        num_workers =0  # Jittor 不支持多进程，设置为 0
